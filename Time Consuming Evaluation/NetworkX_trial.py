...
if __name__ == "__main__":
    ...
    recorder.time_record(func=generate_graph_erdos_renyi, n=100, p=0.2)
    recorder.time_record(func=generate_complete_graph, n=100)
    recorder.time_record(func=generate_directed_gn_graph, n=100)
    ...
    recorder.set_graph_generator_config(directed=False, nodes_num=100, erdos_renyi_probability=0.2)
    recorder.time_record_with_graph(func=get_all_cliques, test_counts=10)
    recorder.time_record_with_graph(func=get_min_edge_dominating_set)
    recorder.time_record_with_graph(func=get_closeness_centrality, test_counts=100)
    recorder.time_record_with_graph(func=get_betweenness_centrality, test_counts=100)
    recorder.time_record_with_graph(func=get_communities_girvan_newman, test_counts=100)
    recorder.time_record_with_graph(func=get_clustering_coefficient)
    recorder.time_record_with_graph(func=get_k_core, K=5)
